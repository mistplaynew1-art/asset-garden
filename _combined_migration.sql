...
ON CONFLICT (slug) DO NOTHING;-- Deposit requests
...
END; $$;CREATE OR REPLACE FUNCTION public.approve_deposit(p_request_id UUID, p_note TEXT DEFAULT NULL)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  END IF;
END $$;
-- ============ Schema (combined from 4 source migrations) ============
CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user');
...
-- Admin approve/reject deposit & withdrawal RPCs
...
-- Seed jackpot rows + game catalog
INSERT INTO public.platform_stats (stat_key, stat_value)
VALUES
  ('jackpot_mini', 1247.83),
  ('jackpot_major', 18920.41),
...
-- Update place_bet to include progressive jackpot contributions
CREATE OR REPLACE FUNCTION public.place_bet(
  p_game_type text,
  p_bet_amount numeric,
  p_multiplier numeric DEFAULT 0,
...
-- Enable realtime on platform_stats
ALTER TABLE public.platform_stats REPLICA IDENTITY FULL;
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER PUBLICATION supabase_realtime ADD TABLE public.platform_stats;
  END IF;
END $$;UPDATE public.games SET slug = 'gates-olympus' WHERE slug = 'olympus';
UPDATE public.games SET slug = 'sweet-bonanza' WHERE slug = 'bonanza';
UPDATE public.games SET slug = 'big-bass'      WHERE slug = 'bigbass';
UPDATE public.games SET slug = 'classic'       WHERE slug = 'classic-slots';-- ============ Schema (combined from 7 source migrations) ============
CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user');
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'platform_stats') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.platform_stats;
  END IF;
END $$;CREATE OR REPLACE FUNCTION public.place_bet(p_game_type text, p_bet_amount numeric, p_multiplier numeric DEFAULT 0, p_payout numeric DEFAULT 0, p_result jsonb DEFAULT '{}'::jsonb, p_server_seed text DEFAULT NULL::text, p_client_seed text DEFAULT NULL::text, p_nonce bigint DEFAULT NULL::bigint)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  UPDATE public.platform_stats SET stat_value = stat_value + (p_bet_amount * 0.002), updated_at = now() WHERE stat_key = 'jackpot_grand';
  RETURN jsonb_build_object('round_id', v_round_id, 'won', v_won, 'payout', p_payout, 'balance', v_balance_after);
END; $function$;-- Reset progressive jackpots to $0. They will accumulate again from real bets via place_bet().
INSERT INTO public.platform_stats (stat_key, stat_value)
VALUES ('jackpot_mini', 0), ('jackpot_major', 0), ('jackpot_grand', 0)
ON CONFLICT (stat_key) DO UPDATE SET stat_value = 0, updated_at = now();-- ============ Schema (combined from 7 source migrations) ============
CREATE TYPE public.app_role AS ENUM ('admin', 'moderator', 'user');
...
-- Test credit (player adds test funds to own wallet, capped at 10000)
CREATE OR REPLACE FUNCTION public.add_test_credit(p_amount NUMERIC)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_user_id UUID; v_balance NUMERIC;
BEGIN
...
-- Jackpot rows seeded then reset to 0; place_bet contributes to them
INSERT INTO public.platform_stats (stat_key, stat_value) VALUES
  ('jackpot_mini', 0),
  ('jackpot_major', 0),
  ('jackpot_grand', 0)
...
-- place_bet (final version with WHERE clause on house_wallet UPDATE)
CREATE OR REPLACE FUNCTION public.place_bet(
  p_game_type text, p_bet_amount numeric, p_multiplier numeric DEFAULT 0,
  p_payout numeric DEFAULT 0, p_result jsonb DEFAULT '{}'::jsonb,
  p_server_seed text DEFAULT NULL::text, p_client_seed text DEFAULT NULL::text, p_nonce bigint DEFAULT NULL::bigint
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE v_user_id UUID; v_balance NUMERIC; v_won BOOLEAN; v_round_id UUID; v_balance_after NUMERIC;
BEGIN
  v_user_id := auth.uid();
  ...
  IF v_won THEN
    ...
    VALUES (v_user_id, 'win', p_payout, v_balance_after, p_game_type || ' win', v_round_id);
  END IF;
  INSERT INTO public.house_wallet (id) VALUES (gen_random_uuid()) ON CONFLICT DO NOTHING;
  UPDATE public.house_wallet SET
    total_bets = total_bets + p_bet_amount,
    total_payouts = total_payouts + p_payout,
    total_bets_today = total_bets_today + p_bet_amount,
...
-- Enable realtime for platform_stats so jackpot tickers update live
ALTER TABLE public.platform_stats REPLICA IDENTITY FULL;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'platform_stats') THEN
...
CREATE TRIGGER update_payment_intents_updated_at BEFORE UPDATE ON public.payment_intents FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();-- Crash bet RPC functions: place_crash_bet and cashout_crash_bet.
